  ...
  const renderLine = (text, num, isMatch, isMatchLine) => {
    ...
    return (
      ...
      >
        ...
        {/* ✏ Edit button — always visible when line is available, not just on hover */}
        {isMatchLine && canEdit && (
          <button
            onClick={e => { e.stopPropagation(); onEdit(key) }}
            style={{
              background: isEditing ? T.orangeLight : T.tealLight,
              border: `1px solid ${isEditing ? T.orangeBorder : T.tealBorder}`,
              borderRadius: 5, color: isEditing ? T.orange : T.teal,
              padding: "2px 9px", fontSize: 10, cursor: "pointer",
              fontFamily: "monospace", flexShrink: 0, marginRight: 8,
              opacity: hov || isEditing ? 1 : 0.45,
...
// Filter type options
const FILTER_TYPES = [
  { value: "all",      label: "All",           icon: "⌕",  desc: "search everything" },
  { value: "function", label: "Function Name",  icon: "ƒ",  desc: "find by function/class name" },
  { value: "module",   label: "Module Name",    icon: "◫",  desc: "find by file/module name" },
]
...
export default function CodeSearch() {
  ...
  const [extFilter,    setExtFilter]    = React.useState(null)
  const [contextLines, setContextLines] = React.useState(0)
  const [filterType,   setFilterType]   = React.useState("all")   // ← NEW: function/module/all
  ...
  React.useEffect(() => {
    const t = setTimeout(() => {
      if (query.trim()) setResults(search(query, { filterType }))
      else setResults([])
    }, 180)
  ...
  const handleSaveEdit = (newContent, lineNumber, file) => {
    ...
    if (query.trim()) setTimeout(() => setResults(search(query, { filterType })), 50)
  }
  ...
  const activeFT = FILTER_TYPES.find(f => f.value === filterType)

  const placeholder = filterType === "function"
    ? "search by function or class name…"
    : filterType === "module"
    ? "search by module / file name…"
    : "search files, symbols, content…"

  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      {/* ── Search bar + filter type selector ── */}
      <div style={{ position: "sticky", top: 0, background: T.bg, paddingBottom: 10, zIndex: 10 }}>

        {/* Filter type tabs — NEW */}
        <div style={{ display: "flex", gap: 4, marginBottom: 8 }}>
          {FILTER_TYPES.map(ft => (
            <button
              key={ft.value}
              onClick={() => { setFilterType(ft.value); setExtFilter(null) }}
              title={ft.desc}
              style={{
                display: "flex", alignItems: "center", gap: 5,
                padding: "5px 12px", borderRadius: T.r, fontSize: 11,
                cursor: "pointer", fontFamily: "monospace",
                border: `1px solid ${filterType === ft.value ? T.brand : T.border}`,
                background: filterType === ft.value ? T.brandLight : "transparent",
                color: filterType === ft.value ? T.brand : T.textHint,
                fontWeight: filterType === ft.value ? 700 : 400,
          ...
          <span style={{ marginLeft: "auto", fontSize: 10, color: T.textHint, fontFamily: "monospace", alignSelf: "center" }}>
            {activeFT?.desc}
          </span>
        </div>
        ...
        <div style={{ position: "relative" }}>
          <span style={{ position: "absolute", left: 13, top: "50%", transform: "translateY(-50%)", color: T.textHint, fontSize: 14 }}>
            {activeFT?.icon || "⌕"}
          </span>
          <input
            ref={inputRef}
        ...
        {availableExts.length > 1 && (
          <div style={{ display: "flex", gap: 5, marginTop: 8, flexWrap: "wrap", alignItems: "center" }}>
            <span style={{ fontSize: 10, color: T.textHint, fontFamily: "monospace" }}>ext:</span>
            <button
      ...
      {!query && (
        <div style={{ textAlign: "center", marginTop: 60, color: T.textHint, fontSize: 12 }}>
          <div style={{ fontSize: 32, marginBottom: 10 }}>{activeFT?.icon || "⌕"}</div>
          <div style={{ marginBottom: 6 }}>
            {filterType === "function" && "Type a function or class name to find it across all files"}
            {filterType === "module"   && "Type a module or file name to locate it"}
            {filterType === "all"      && "type to search across all files"}
          </div>
      ...
      {query && totalMatches === 0 && (
        <div style={{ textAlign: "center", marginTop: 60, color: T.textHint, fontSize: 12 }}>
          ...
          {filterType !== "all" && <div style={{ marginTop: 6, fontSize: 11 }}>try switching filter to "All"</div>}
        </div>
      )}
          ...
          return (
            <div key={filePath} style={{ marginBottom: 10, background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.rMd, overflow: "hidden" }}>
              ...
              >
                ...
                {filterType === "function" && fileResults[0]?.matchedName && (
                  <span style={{ background: T.tealLight, border: `1px solid ${T.tealBorder}`, borderRadius: 10, color: T.teal, fontSize: 10, padding: "2px 8px", flexShrink: 0 }}>
                    ƒ {fileResults.map(r => r.matchedName).join(", ")}
                  </span>
                )}
