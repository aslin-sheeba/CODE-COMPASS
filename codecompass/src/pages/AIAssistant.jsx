...
const MODEL_LABEL = "llama-3.1-8b-instant"
...
function buildSystemPrompt(files) {
  ...
  const summaries = top.map(f =>
    `FILE: ${f.path}\n  imports: ${(f.imports || []).slice(0, 8).join(", ") || "none"}\n  stress: ${f._meta?.stressScore ?? 0} | incoming: ${f._meta?.incoming ?? 0} | lines: ${f.lines ?? "?"}`
  ).join("\n\n")
...
export default function AIAssistant() {
  ...
  const handleSend = async (text) => {
    ...
    try {
      // Build conversation history for aiService (skip intro, skip current message)
      const history = newMsgs
        .slice(1)       // skip BOT_INTRO — it's UI-only, not a real turn
        .slice(0, -1)   // exclude the message we just added (that becomes the prompt arg)
        .filter(m => m.role === "user" || m.role === "assistant")
        .map(m => ({ role: m.role, content: m.text || "" }))

      const reply = await askAI(msg, { history, systemPrompt: buildSystemPrompt(files) })
      ...
      <div style={{ padding: "12px 20px", borderBottom: `1px solid ${T.border}`, background: T.surface, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 10, color: T.textHint, fontFamily: "monospace" }}>{MODEL_LABEL} · {files.length} files loaded</div>
          </div>
        </div>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 4 }}>
            {QUICK.map(p => (
              <button key={p} onClick={() => handleSend(p)}
                style={{ padding: "5px 12px", borderRadius: 20, border: `1px solid ${T.border}`, background: T.surface, color: T.textSub, cursor: "pointer", fontSize: 11, fontFamily: "monospace", transition: "all 0.12s" }}
