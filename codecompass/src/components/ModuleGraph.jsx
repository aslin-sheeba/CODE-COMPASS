...
function GraphSearch() {
  ...
  React.useEffect(() => {
    ...
    setResults(files.filter(f => f.path.toLowerCase().includes(query.toLowerCase())).slice(0,8))
  }, [query, files])
...
export default function ModuleGraph({ width = 1000, height = 600, activeCycle }) {
  ...
  useEffect(() => {
    ...
    // Detect cycles
    const visited = new Set(), stack = new Set(), edges = new Set()
    const dfs = id => {
  ...
  useEffect(() => {
    ...
    simRef.current = sim
    let raf
    const render = () => { setNodes([...nodes]); raf = requestAnimationFrame(render) }
    render()
  ...
  const onWheel = e => { e.preventDefault(); const k=Math.max(0.3,Math.min(3,transform.k-e.deltaY*0.001)); setTransform(t=>({...t,k})) }
  const onPanStart = e => {
        ...
        <svg width={width} height={height} onWheel={onWheel} onMouseDown={onPanStart}
          ...
          <g transform={`translate(${transform.x},${transform.y}) scale(${transform.k})`}>
            ...
            {links.map((l,i) => {
              ...
              const visible = hoveredNode ? (s.id===hoveredNode.id||t.id===hoveredNode.id) : (!highlightedFile||treeNodes.has(s.id))
              return (
                <line key={`${key}-${i}`} x1={s.x} y1={s.y} x2={t.x} y2={t.y}
                  stroke={isCycle?T.red:T.borderHover} strokeWidth={isCycle?2:1.5}
                  strokeOpacity={visible?0.8:0.07} markerEnd={isCycle?"url(#arr-cycle)":"url(#arr)"} />
              )
            })}
            {/* Nodes */}
            {nodes.map((n,idx) => {
              ...
              const visible  = hoveredNode
                ? n.id===hoveredNode.id||links.some(l=>{ const s=typeof l.source==="object"?l.source.id:l.source,t=typeof l.target==="object"?l.target.id:l.target; return (s===hoveredNode.id&&t===n.id)||(t===hoveredNode.id&&s===n.id) })
              ...
              return (
                <g key={`${n.id}-${idx}`}>
                  ...
                  <circle cx={n.x} cy={n.y} r={n.radius} fill={nodeColor(n.stress)} opacity={visible?1:0.1}
                    stroke={isUnused?"#8b5cf6":"none"} strokeWidth={isUnused?2:0} style={{ cursor:"pointer" }}
